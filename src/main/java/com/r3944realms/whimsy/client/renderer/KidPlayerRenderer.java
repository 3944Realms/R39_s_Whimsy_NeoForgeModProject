...
import com.r3944realms.whimsy.client.mdoel.KidPlayerModel;
...
@OnlyIn(Dist.CLIENT)
@Deprecated
public class KidPlayerRenderer extends LivingEntityRenderer<AbstractClientPlayer, KidPlayerModel<AbstractClientPlayer>> {

    public KidPlayerRenderer(EntityRendererProvider.Context pContext, boolean pUSeSlimModel) {
        super(pContext, new KidPlayerModel<>(pContext.bakeLayer(pUSeSlimModel ? ModelLayers.PLAYER_SLIM : ModelLayers.PLAYER), pUSeSlimModel), 0.5F);
...
//        this.addLayer(new NewArrowLayer<>(pContext, this));
...
//        this.addLayer(new NewParrotOnShoulderLayer<>(this, pContext.getModelSet()));
//        this.addLayer(new NewSpinAttackEffectLayer<>(this, pContext.getModelSet()));
...
//        net.neoforged.neoforge.common.NeoForge.EVENT_BUS.post(new RenderKidPlayerEvent.Post(pEntity, this, pPartialTicks, pPoseStack, pBuffer, pPackedLight));
    ...
    private void setModelProperties(AbstractClientPlayer pClientPlayer) {
        KidPlayerModel<AbstractClientPlayer> playerModel = this.getModel();

...
@OnlyIn(Dist.CLIENT)
public class NewBeeStingerLayer<T extends LivingEntity, M extends NewKidPlayerModel<T>> extends NewStuckInBodyLayer<T, M> {
    ...
    public NewBeeStingerLayer(LivingEntityRenderer<T, M> pRenderer) {
        super(pRenderer);
    }
    ...
    protected void renderStuckItem(
        ...
        float f1 = (float)(Math.atan2(pX, pZ) * 180.0F / (float)Math.PI);
        float f2 = (float)(Math.atan2(pY, f) * 180.0F / (float)Math.PI);

package com.r3944realms.whimsy.gamerule.ClientRender;
...
import static com.r3944realms.whimsy.gamerule.Gamerules.gamerulesBooleanValuesClient;
...
public class MustOthersHiddenNameTag {
    ...
    public static final BiConsumer<MinecraftServer, GameRules.BooleanValue> NOTICE = (MinecraftServer server, GameRules.BooleanValue value) -> {
        server.getPlayerList().getPlayers().forEach(player -> {
            gamerulesBooleanValuesClient.put(ID, value.get());
            PacketDistributor.sendToPlayer(player, new BooleanGameRuleValueChangeData(ID, value.get()));//通知客户端
        });
    };
    @SubscribeEvent
    public static void commonSetup(final FMLCommonSetupEvent event) {
        gamerulesBooleanValuesClient.put(ID, DEFAULT_VALUE);
        GAMERULE_REGISTRY.registerGamerule(ID, CATEGORY, DEFAULT_VALUE, NOTICE);
    }

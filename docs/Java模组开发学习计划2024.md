...
ChannelDeulexHandler既可以作为入站和出站处理器其中的一个方法userEventTriggered
...
类用final修饰 保证该类的方法不能被覆盖，防止子类无意间破坏不可变性

属性用final修饰保证其可读性。不能被修改
...
（Simple Responsibliity Pincipe, SRP）是最简单的对象设计原则，用于控制类的粒度大小

> 只包含单一职责，该职责被完整的封装在一个类中

##### 开闭原则

（Open Close Principle）重要的面向对象设计原则

> 软件实体应当<font color="#efee3f">(对提供方)</font>对扩展开放，对<font color="#efee3f">(对调用者)</font>修改关闭

##### 里氏替换原则

(Liskov Subsitution Principle) 对子类型的特别定义【一种建议】

> 所以引用基类的地方必须能够透明地使用其子类对象

子类可扩展父类的功能，但不能改变父类原有的功能：

1. 子类可以实现父类的抽象方法，但不能覆盖父类的非抽象方法
2. 子类可以自增自己特有的方法
3. 当子类重载父类的方法时，方法的前置条件（输入/入参）要比父方法的输入参数更加宽松
4. 当子类实现父类的方法时（重写/重载或实现抽象方法），方法的后置条件（输出/返回值）要比父类严格或一样
...
(Interface Segregation Principle,ISP) 实际是对接口的细化
...
(Composite Reuse Principle) 委派

> 优先使用对象组合，而不是提供继承来达到复用的目的

降低耦合度 ,提高安全性（不关心具体的类的字段与方法，<s>直接</s>调用）

##### 迪米特法则

(Law of Demeter) 最少知识原则，是对程序内部数据交互的限制 [ **小气法则（** ]

> 每一个软件单位对其它软件单位都只有最少的知识，而且局限于那些于本单位密切相关的软件单位
...
XXXBuilder（例如Netty的BootStrap类）

###### 单例模式

Singleton

饿汉式（类加载创建） & 懒汉式（延迟加载【<font color="#ff0000">会有线程安全问题需考虑</font>】）

```Java
    public static Singleton getInstance() {
        if(INSTANCE == null) {
            synchronized (BooleanBigLists.Singleton.class) {
                if(INSTANCE == null) INSTANCE = new Singleton();// 内部还需要进行一次检查，双重检查锁定
            }
        }
        return INSTANCE;
    }
//INSTANCE 可见性 volatile
```
...
public class Signleton {
    private Singleton() {}
    private static class Holder { //静态内部类持有单例对象，但是根据类加载特性，我们仅使用Singleton类，不会对静态内部类进行初始化
...
原型对象为模板，通过克隆操作实现产生更多的对象

 浅拷贝 和 深拷贝【基本类型和引用类型】

在Java中，可以使用Colseable接口的提供的拷贝机制来实现原型模式
在Java中如何实现深拷贝（原生的native clone 方法只拷贝出来的对象其内部字段属性还是对原来的引用，不是深拷贝）
...
不改变对象本身功能的基础上，给对象进行额外的行为【通过组合实现，而不是传统的继承关系】
...
除了自己手动写代理类外，JDK还提供了一种动态代理机制，不需要自己来手动编写继承关系创建代理类，它可以在运行时通过反射机制来自动生成代理类
...
除了JDK之外第三方也有对应的动态代理方式 CGlib框架（底层ASM框架，可以直接去修改类中的字节码）

##### 外观模式

Facade门面 通过门面完成多个事件 【降低类与类的直接关联程度，简化了流程】

...
> windows系统下命令行可以用tasklist 命令来查看进程，taskkill来杀死进程；
>
> 也可以用任务管理器来查看进程和线程数，用其杀死进程
...
```java
@FunctionalInterface//Runnable接口有**@FunctionalInterface**注解，凡有此注解的可以用lambada简化
...
| 方法                                        | 说明                                                         |
| ------------------------------------------- | ------------------------------------------------------------ |
| public void start()                         | 启动一个新线程，Java虚拟机调用此线程的 run 方法【让线程进入就绪状态，只能调用一次】 |
...
* 调用 yield 会让提示线程调度器让出当前线程对 CPU 的使用，使程序从Running状态进入到Runnable(**就绪**)状态
* 具体的实现依赖于操作系统的任务调度器
* **会放弃 CPU 资源，锁资源不会释放**
...
**可以修改获取打断状态方法，使用 <u>`Thread.interrupted()`</u>，清除打断标记**
...
![image-20240413174648383](C:\Users\f2561\AppData\Roaming\Typora\typora-user-images\image-20240413174648383.png)
...
**程序计数器（Program Counter Register）**：记住下一条 JVM 指令的执行地址，是<u>线程私有的</u>
...
进程的状态参考操作系统（五种状态）：创建态、就绪态、运行态、阻塞态、终止态
...
*  **情况01：** NEW → RUNNABLE：当调用 t.start() 方法时，由 NEW → RUNNABLE

* **情况02：**  RUNNABLE <--> WAITING：

  **线程**用synchronized(obj) 获取了对象锁后
  ...
  * **情况03:** 当前线程调用 t.join() 方法，注意是当前线程在 t 线程对象的监视器上等待

  * 情况04: 当前线程调用 LockSupport.park() 方法

* **情况05\06\07\08:** RUNNABLE <--> TIMED_WAITING：调用 obj.wait(long n) 方法、当前线程调用 t.join(long n) 方法、当前线程调用 Thread.sleep(long n) ,LockSupport.parkNanos(long nanos), LockSupport.parkUntil(long millis)
...
* 当锁对象第一次被线程获得的时候进入偏向状态，标记为 101，同时**使用操作将线程 ID 记录到 Mark Word**。如果 [CAS 操作](https://zhuanlan.zhihu.com/p/101430930)成功，这个线程以后进入这个锁相关的同步块，查看这个线程 ID 是自己的就表示没有竞争，就不需要再进行任何同步操作

* 当有另外一个线程去尝试获取这个锁对象时，偏向状态就宣告结束，此时撤销偏向（Re voke Bias）后恢复到未锁定或轻量级锁状态
...
* 添加 VM 参数 `-XX:-UseBiasedLocking` 禁用偏向锁（“XX:**-**”中"- "表示禁用）

撤销偏向锁的状态：

* **调用对象的 hashCode**：偏向锁的对象 MarkWord 中存储的是**线程 id(操作系统层面)**，调用 hashCode 导致偏向锁被撤销【偏向对象对象头没有存放Hashcode空间，所以会撤销偏向锁。对于轻量级锁，HashCode可存放在线程锁记录里，而对于重量级锁，其HashCode存放在monitor对象里】
...
​	每个线程都有自己的一个Parker对象**（底层是C代码来实现的）**，由三个部分组成`_counter`,`_cond`,`_mutex`
...
常见线程安全类：String、Integer、StringBuffer、Random、Vector、Hashtable、java.util.concurrent （简称**JUC**）包
...
指令重排在多线程下出现异常情况的现象需要大量的测试才能够复现：

可以借助java并发测压工具 [**jcstress**](https://wiki.openjdk.java.net/display/CodeTools/jcstress)
